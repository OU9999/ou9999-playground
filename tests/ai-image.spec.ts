...
test("ai 이미지 생성 테스트 - stable-diffusion", async ({ page }) => {
  ...
  await page.waitForSelector("img[alt='ai-image']", { timeout: 60000 });
...
test("ai 이미지 생성 테스트 - sdxl", async ({ page }) => {
  // index 페이지에서 시작 (playwright.config.ts에서 webServer를 통해 baseURL을 설정.)
  await page.goto("/");
  ...
  await page.locator("#\\:r1\\:\\-form-item").click();
  await page.waitForSelector("[data-radix-popper-content-wrapper]");
  await page.click("//div[span[text()='sdxl (art style)']]");
...
test("ai 이미지 생성 테스트 - sdxl custom form", async ({ page }) => {
  // index 페이지에서 시작 (playwright.config.ts에서 webServer를 통해 baseURL을 설정.)
  await page.goto("/");
  ...
  await page.click("text=sdxl custom form");

  // id advanced-submit
  await page.click("#advanced-submit");
